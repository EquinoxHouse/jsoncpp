...
namespace Json {
    ...
    ValueInternalLink::~ValueInternalLink() {
        for (int index = 0; index < itemPerLink; ++index) {
            if (!items_[index].isItemAvailable()) {
                if (!items_[index].isMemberNameStatic()) free(keys_[index]);
            } else
                break;
        }
    }
    ...
    */
    ...
    bool ValueInternalMap::reserve(BucketIndex newItemCount) {
        ...
        //   BucketIndex idealBucketCount = (newItemCount + ValueInternalLink::itemPerLink) /
        //   ValueInternalLink::itemPerLink;
        return true;
    }

    const Value* ValueInternalMap::find(const char* key) const {
        if (!bucketsSize_) return 0;
        HashKey hashedKey = hash(key);
        BucketIndex bucketIndex = hashedKey % bucketsSize_;
        for (const ValueInternalLink* current = &buckets_[bucketIndex]; current != 0; current = current->next_) {
            for (BucketIndex index = 0; index < ValueInternalLink::itemPerLink; ++index) {
                if (current->items_[index].isItemAvailable()) return 0;
                if (strcmp(key, current->keys_[index]) == 0) return &current->items_[index];
            }
        }
        return 0;
    }
        ...
        if (bucketsSize_) {
            ...
            for (ValueInternalLink* current = &buckets_[bucketIndex]; current != 0;
                ...
                for (index = 0; index < ValueInternalLink::itemPerLink; ++index) {
                    if (current->items_[index].isItemAvailable()) return setNewItem(key, isStatic, current, index);
                    if (strcmp(key, current->keys_[index]) == 0) return current->items_[index];
                }
            }
        }
    ...
    void ValueInternalMap::doActualRemove(ValueInternalLink* link, BucketIndex index, BucketIndex bucketIndex) {
        ...
        {
            if (lastLink->items_[lastItemIndex].isItemAvailable()) break;
        }

        BucketIndex lastUsedIndex = lastItemIndex - 1;
        Value* valueToDelete = &link->items_[index];
        Value* valueToPreserve = &lastLink->items_[lastUsedIndex];
        if (valueToDelete != valueToPreserve) valueToDelete->swap(*valueToPreserve);
    ...
    ValueInternalLink*& ValueInternalMap::getLastLinkInBucket(BucketIndex bucketIndex) {
        if (bucketIndex == bucketsSize_ - 1) return tailLink_;
        ValueInternalLink*& previous = buckets_[bucketIndex + 1].previous_;
        if (!previous) previous = &buckets_[bucketIndex];
        return previous;
    }
    ...
    Value& ValueInternalMap::unsafeAdd(const char* key, bool isStatic, HashKey hashedKey) {
        JSONCPP_ASSERT_MESSAGE(bucketsSize_ > 0, "ValueInternalMap::unsafeAdd(): internal logic error.");
        BucketIndex bucketIndex = hashedKey % bucketsSize_;
        ValueInternalLink*& previousLink = getLastLinkInBucket(bucketIndex);
        ValueInternalLink* link = previousLink;
        BucketIndex index;
        for (index = 0; index < ValueInternalLink::itemPerLink; ++index) {
            if (link->items_[index].isItemAvailable()) break;
        }
    ...
    void ValueInternalMap::increment(IteratorState& iterator) {
        JSONCPP_ASSERT_MESSAGE(iterator.map_, "Attempting to iterator using invalid iterator.");
        ++iterator.itemIndex_;
        if (iterator.itemIndex_ == ValueInternalLink::itemPerLink) {
            JSONCPP_ASSERT_MESSAGE(
                iterator.link_ != 0, "ValueInternalMap::increment(): attempting to iterate beyond end."
            );
            iterator.link_ = iterator.link_->next_;
            if (iterator.link_ == 0) incrementBucket(iterator);
    ...
    void ValueInternalMap::decrement(IteratorState& iterator) {
        if (iterator.itemIndex_ == 0) {
            JSONCPP_ASSERT_MESSAGE(iterator.map_, "Attempting to iterate using invalid iterator.");
            if (iterator.link_ == &iterator.map_->buckets_[iterator.bucketIndex_]) {
                JSONCPP_ASSERT_MESSAGE(iterator.bucketIndex_ > 0, "Attempting to iterate beyond beginning.");
                --(iterator.bucketIndex_);
            }
    ...
    const char* ValueInternalMap::key(const IteratorState& iterator) {
        JSONCPP_ASSERT_MESSAGE(iterator.link_, "Attempting to iterate using invalid iterator.");
